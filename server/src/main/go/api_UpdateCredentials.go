...
func (s *Server) UpdateCredentials(c *gin.Context) {
	// get the email, site, new password and validation code from the request body
	...
	err = s.updateCredentialsIntoCredsDb(email, site, userid, pwdCipherText)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"msg": "Server error",
		})
	...
	c.JSON(http.StatusOK, gin.H{
		"msg": "successfully updated credentials for " + email + ", " + site,
	})
}
